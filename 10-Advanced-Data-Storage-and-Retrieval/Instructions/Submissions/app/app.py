...
def get_most_active_tobs():
    data = sqlHelper.get_most_active_tobs()
    return jsonify(json.loads(data.to_json(orient="records")))

@app.route("/api/v1/<start_date>/<end_date>")
def get_temp_date_range():
    data = sqlHelper.get_temp_date_range()
    return jsonify(json.loads(data.to_json(orient="records")))

@app.route("/api/v1/<start_date>")
def get_temp_data_for_date():
    data = sqlHelper.get_temp_data_for_date()
    return jsonify(json.loads(data.to_json(orient="records")))
